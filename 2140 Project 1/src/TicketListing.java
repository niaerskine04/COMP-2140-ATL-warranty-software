...
public class TicketListing extends JFrame { // window to display Add ticket option to alt team
    ...
    public TicketListing() {
        ...
        cmdAddTicket = new JButton("Add New Customer Ticket");
        ...
        cmdAddTicket.addActionListener(new TicketListener());

        pnlDisplay.add(prompt);

        pnlCommand.add(cmdAddTicket);
    ...
    public void addToTable(Ticket t) {
        String[] record = { "" + t.getTicketNumber() + "" + t.getTitle() + " " + t.getName() + " " + t.getContactInfo()
                + " " + t.getDescription() + " " + t.getPriority() + " " + t.getStatus() };
        tablemodel.addRow(record);
    }

    /**
     * Load Ticket Methods
     * loads ticket detaiils from the file so that the ticket can be added to the
     * ticket table
     * 
     * @param fileName
     * @return
